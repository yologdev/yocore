...
- **`mcp/`** — Stdio JSON-RPC server implementing Model Context Protocol. 5 tools for AI assistants to query memories, context, and skills.
- **`ai/`** — AI feature modules (title generation, memory extraction, skill discovery, marker detection, export generation, ranking). Uses Claude Code CLI as subprocess. `AiTaskQueue` (semaphore) limits concurrency. Title generation works in both storage modes; other features require DB. Export uses fire-and-forget pattern with SSE delivery.
